...
class MancalaApp(App):
    ...
    def on_start(self):
        ...
        if self.store.exists('save_matrix'):
            ...
            self.first_step       = [True if self.store.get('first_step')['value'][0] == 1 else False, True if self.store.get('first_step')['value'][1] == 1 else False]
    ...
    def turn(self, player):
        ...
        if not self.player_AI or (player == 0 and self.player_AI):
            ...
            self.rotate_btn[player].disabled = False if ((self.first_step == [False, True] and player == 1) or (self.first_step == [True, False] and player == 0)) and self.game_seeds == 72 else True
    ...
    def balda(self):
        ...
        is_rotate_disabled = False if self.first_step == [False, True] and self.game_seeds == 72 and len(self.fields[0][6].seeds) == 2 else True
        ...
        elif self.first_step == [True, True] or random.randint(1, 100) <= 10:  # game first move random or 10% chance move random ;)
    ...
    def move_analyze(self, get_matrix, player, index):
        ...
        while temp_hand > 0:
            ...
            # end move
                ...
                # get alien seeds
                if matrix[row][col] == 1 and row == player and not ((row == 0 and col == 6) or (row == 1 and col == 0)) and matrix[(row + 1) if player == 0 else (row - 1)][(col + 1) if player == 0 else (col - 1)] > 0:
                ...
                # game over
                elif (matrix[0][0]+matrix[0][1]+matrix[0][2]+matrix[0][3]+matrix[0][4]+matrix[0][5] == 0) or (matrix[1][1]+matrix[1][2]+matrix[1][3]+matrix[1][4]+matrix[1][5]+matrix[1][6] == 0):
                    return (False, matrix)
    ...
    def after_move(self, *args):
        if self.hand:
            ...
            if len(self.hand) > 0:
                ...
                # move alien side
                elif not self.fields[self.board_row][(self.board_col - 1) if self.player_turn == 0 else (self.board_col + 1)].is_ambar and self.board_row != self.player_turn:
                    self.board_col = (self.board_col - 1) if self.player_turn == 0 else (self.board_col + 1)

                # alien ambar (before 1 step)
                elif self.fields[self.board_row][(self.board_col - 1) if self.player_turn == 0 else (self.board_col + 1)].is_ambar and self.board_row != self.player_turn:
                    self.board_row = (self.board_row - 1) if self.player_turn == 0 else (self.board_row + 1)
                    self.board_col = (self.board_col - 1) if self.player_turn == 0 else (self.board_col + 1)
            ...
            # end move
                ...
                # get alien seeds
                if len(self.fields[self.board_row][self.board_col].seeds) == 1 and self.board_row == self.player_turn and not self.fields[self.board_row][self.board_col].is_ambar and len(self.fields[(self.board_row + 1) if self.player_turn == 0 else (self.board_row - 1)][(self.board_col + 1) if self.player_turn == 0 else (self.board_col - 1)].seeds) > 0:
                    self.hand = self.fields[(self.board_row + 1) if self.player_turn == 0 else (self.board_row - 1)][(self.board_col + 1) if self.player_turn == 0 else (self.board_col - 1)].seeds
                    self.fields[(self.board_row + 1) if self.player_turn == 0 else (self.board_row - 1)][(self.board_col + 1) if self.player_turn == 0 else (self.board_col - 1)].seeds = []
